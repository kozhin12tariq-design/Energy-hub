...
fprintf('\n--- Same sweep, verified on IEEE 33 (ONE hub at bus %d, unity power factor) ---\n', sysNet.hostBus);
fprintf('%-10s %12s %12s %14s %14s %14s\n', ...
    'nSegments', 'min V (pu)', 'hrs<0.95', 'losses kWh', 'MILP(s)', 'MILP+PF(s)');
fprintf('%-10s %12.4f %12s %14.1f %14s %14s\n', 'base', ...
    nvBase.base_minV, '24.00', nvBase.base_lossEnergy_kWh, '-', '-');
for i = 1:nS
    fprintf('%-10d %12.4f %12.2f %14.1f %14.5f %14.5f\n', segCounts(i), ...
        netMinV(i), netHrsBelow(i), netLossEnergy(i), solveTime(i), solveTime(i)+verifyTime(i));
end
...
if vSpread < 5e-3 && lSpread < 50
    fprintf(['\nThe network outcomes are essentially INSENSITIVE to segment count, and that is\n' ...
        ...
        'segmentation earns its keep on COST accuracy, which is where the n=1/n=2 rows are\n' ...
        'badly wrong (%+.2f%% and %+.2f%% realized-vs-planned gaps) even though their voltage\n' ...
        'and loss figures are indistinguishable from n=36''s. Accuracy in one metric does not\n' ...
        'imply accuracy in the other, and this sweep is the evidence for that claim.\n'], ...
        vSpread, lSpread, ...
        100*(realizedCost(1)-plannedCost(1))/abs(plannedCost(1)), ...
